...
if __name__ == '__main__':
    ...
    cfg.model = "./log/{}/best.ckpt".format(env_)
    
    # logs
    logs_name = 'logs'
    log_save_path = os.path.join(os.getcwd(), logs_name)
    if not os.path.exists(log_save_path):
        os.makedirs(log_save_path)

    day_name = date.today()
    log_save_path = os.path.join(log_save_path, str(day_name))
    if not os.path.exists(log_save_path):
        os.makedirs(log_save_path)

    current_time = datetime.datetime.now()
    formatted_time = current_time.strftime("%Y-%m-%d-%H:%M:%S")
    sys.stdout = open(os.path.join(log_save_path,formatted_time+'-logs.txt'), "w")

    comment=f'bath_size={batch_size} lr={learning_rate} weight_decay={weight_decay} eval_ratio={eval_ratio} neg_ratio={neg_ratio}'
    print(comment)
    ...
    for name in sequs_test:
        ...
        save_ply_cls(os.path.join(save_ply_folder, name+'_cls.ply'), pc_out)
